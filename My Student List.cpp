...
void sync_data(student students[], int &size);
void countdown()
{
...
void raw_data(student students[], int size, bool input_check);
int main()
{	
	int mode;
	int size;
	...
	cout << "Choose your service!" << endl
		 ...
		 << "9.Write into raw data file." << endl
		 << "10.Load raw data file." << endl
		 << "11.Exit." << endl;
	cin >> mode;
	switch (mode)
	{
	...
	case 9:
		if (input_check == false)
		{
			...
			cout << "Creating raw data file." << endl;
			raw_data(students, size, input_check);
			cout << "Raw data file created." << endl;
			goto repeat_choosing;
		}
	case 10:
		if (input_check == false){
			sync_data(students, size);
			cout << "Data loaded!" << endl;
            input_check = true;
			goto repeat_choosing;
		}
		else{
			cout << "This action is not allowed because data would be overload, please start new session." << endl;
            goto repeat_choosing;
		}
	case 11:
		countdown();
...
repeat_input:
	...
	repeat_phys:
		...
		cout << "Please start a new session" << endl;
	}
}
void add_students(student students[], int &size, bool input_check)
{
...
void docx(student students[], int size, bool input_check){
	...
	for (int i = 0; i < size; i++)
	{
		...
		students_list << "ID: " << students[i].id << endl;
...
void raw_data(student students[], int size, bool input_check){
	ofstream students_list("Students_list_raw_data.docx");
	for (int i = 0; i < size; i++)
	{
		students_list << students[i].position << endl;
		students_list << students[i].name << endl;
		students_list << students[i].id << endl;
		students_list << students[i].age << endl;
		if (students[i].gender == 'M')
		{
			students_list << "Male" << endl;
		}
		...
		students_list << students[i].math_score << endl;
		students_list << students[i].chem_score << endl;
		students_list << students[i].phys_score << endl;
		students[i].avg = (students[i].math_score + students[i].chem_score + students[i].phys_score) / 3.0;
		students_list << students[i].avg << endl;
		if (students[i].avg >= 9.0)
		{
			students[i].overall = "Excellent";
		}
		...
		students_list << students[i].overall << endl;
	}
	students_list.close();
}
void sync_data(student students[], int size, bool input_check){
	cout << "Please enter number of students in raw data file." << endl;
    cin >> size;
	string path;
	...
	cin.ignore(1000,'\n');
	getline(cin, path);
	ifstream students_list(path);
	int i = 0;
	int j = size;
   while(j-- != -1){
        students_list >> students[i].position;
        getline(students_list, students[i].name, '\n'); 
        getline(students_list, students[i].id, '\n'); 
        students_list >> students[i].age;
        students_list >> students[i].gender;
        students_list >> students[i].math_score;
        students_list >> students[i].chem_score;
        students_list >> students[i].phys_score;
        students_list >> students[i].avg;
        getline(students_list, students[i].overall, '\n'); 
		i++;
    }
	students_list.close();
}