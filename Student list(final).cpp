...
struct student
{
	...
	float avg;
	string overall;
	string temp; //when read the raw data file, overall of the previous would be reread, so temp will be use to store that reread string
};
...
void delete_by_id(student students[], int &size, bool input_check);
void output_raw_data(student students[], int size, bool input_check);
...
cout << "Press any key to continue..." << endl;
	...
	cout << "Choose your service!" << endl
		 ...
		 << "9.Delete a student by id." << endl
		 << "10.Write into raw data txt file." << endl
         << "11.Load raw data txt file." << endl
		 << "12.Exit" << endl;
	cin >> mode;
	switch (mode)
	{
	...
	case 9:
		if (input_check == false || size == 0){
			...
			delete_by_id(students,size,input_check);
			goto repeat_choosing;
		}
	case 10:
		if (input_check == false || size == 0){
			...
			output_raw_data(students, size, input_check);
			goto repeat_choosing;
		}
    case 11:
        if (input_check == false || size == 0){
            sync_data(students, size, input_check);
            goto repeat_choosing;
        }
        else{
...
void docx(student students[], int size, bool input_check){
	ofstream students_list("Students list.txt");
	for (int i = 0; i < size; i++)
	{
	...
	for(int i = 0; i < size; i++){
		if(students[i].id == id_check){
			...
			break;
				...
				cout << "You have aborted the function!" << endl;
				break;
			}
		}
	}
...
void sync_data(student students[], int &size, bool &input_check){
	...
	getline(cin,path);
	ifstream data(path);
    cout <<"Enter number of students in the file: " << endl;
    cin >> size;
	if(!data.is_open())
	{
		...
		students[i].position = i + 1;
       	getline(data,students[i].name);
		getline(data,students[i].id);
        data >> students[i].age;
        data >> students[i].gender;
        data >> students[i].math_score;
        data >> students[i].chem_score;
        data >> students[i].phys_score;
        data >> students[i].avg;
        getline(data, students[i].overall);
		getline(data, students[i].temp);
        i++;
        }
	}
	input_check = true;
